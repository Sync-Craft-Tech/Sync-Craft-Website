      ...
      <div className="hero px-4 md:px-6">
        <div className="flex flex-col items-center space-y-4 text-center mb-12">
          ...
          <p className="max-w-[900px] text-muted-foreground md:text-xl">
            Websites, web-apps, and consulting solutions we delivered for happy clients.
          </p>
        </div>
        ...
        <div className="relative w-full">
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {projects.map((proj, idx) => (
                <CarouselItem
                  key={idx}
                  className="pl-2 md:pl-4 basis-[85%] sm:basis-1/2 lg:basis-1/3 transition-transform duration-700 will-change-transform contain-content"
                  style={{
                ...
                >
                  <Card className="h-full overflow-hidden rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300">
                    <CardContent className="p-0">
                      <div className="aspect-video relative overflow-hidden">
                        ...
                        <div className="absolute left-4 bottom-4 right-4 text-left text-white pointer-events-auto">
                          <h3 className="font-semibold text-lg drop-shadow-sm">{proj.title}</h3>
                          <p className="text-sm opacity-90 mt-1 drop-shadow-sm">{proj.description}</p>
                        </div>
                      </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            {projects.map((_, i) => (
              <span key={i} className="h-2 w-8 rounded-full bg-background/60 opacity-60" aria-hidden />
            ))}
          </div>
        </div>
      </div>
